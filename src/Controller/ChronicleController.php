...
class ChronicleController extends AbstractController
{
  ...
  public function editNote(Request $request, Note $note): Response
  {
    ...
    $form = $this->createForm(NoteType::class, $note, [
      'categories' => $this->dataService->getChronicleNotesCategory($note->getChronicle(), $user),
    ]);
    $form->handleRequest($request);
    ...
    if ($form->isSubmitted() && $form->isValid()) {
      ...
      return $this->redirectToRoute('chronicle_show', ['id' => $note->getChronicle()->getId()], Response::HTTP_SEE_OTHER);
    }
    return $this->renderForm('notes/new.html.twig', [
      'note' => $note,
      'form' => $form,
