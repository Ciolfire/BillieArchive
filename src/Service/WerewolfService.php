...
class WerewolfService
{
  ...
  public function firstChange(Character $character, FormInterface $form): bool
  {
    $connection = $this->dataService->getConnection();
    $data = $form->getData();
    
    if (!$data['auspice'] instanceof Auspice) {
    ...
    $nativeQuery = $connection->prepare("UPDATE `characters` SET type='werewolf' WHERE id = :id");
    $nativeQuery->bindValue('id', $character->getId());
    $nativeQuery->executeStatement();
    // ... as a Werewolf for the rest of her life
    // $nativeQuery = $connection->prepare("INSERT IGNORE INTO `werewolf`(`id`, `path_id`, `order_id`, `gnosis`, `mana`) VALUES (:id, :path, :order, 1, $startingMana)");
    // $nativeQuery->bindValue('id', $character->getId());
    // $nativeQuery->bindValue('path', $data['path']->getId());
    if ($data['tribe'] instanceof Tribe) {
      $nativeQuery->bindValue('tribe', $data['tribe']->getId());
    } else {
      $nativeQuery->bindValue('tribe', null);
    }
    $nativeQuery->executeStatement();
    // // We force the change to the manager, to avoid conflict from memory (?)
    $this->dataService->reset();
    $werewolf = $this->dataService->find(Werewolf::class, $character->getId());
    // Bonus attribute and starting Arcana
    // $werewolf->addAttribute($data['path']->getAttribute()->getIdentifier(), 1);
    // $this->addArcana($werewolf, $arcana);

    // Should we really ? There is potential loss of informations there
    // $werewolf->cleanLesserTemplates();
    
    if ($werewolf->getLesserTemplate()) {
      $werewolf->getLesserTemplate()->setIsActive(false);
    }
    $this->dataService->save($werewolf);
   ...
   */
  public function getSpecial(Werewolf $mage): array
  {
  ...
  // private function filterArcana(array $arcana, Werewolf $mage): array
  // {
  ...
  // private function filterRotes(array $rotes, Werewolf $mage): array
  // {
  ...
  //       !is_null($rote->getWerewolfOrder()) && $rote->getWerewolfOrder() != $mage->getOrder() 
  //     ) {
  //       unset($rotes[$key]);
  //     }
  //   }
  ...
  // public function handleEdit(Werewolf $character, array $data): void
  // {
  ...
  // public function addArcana(Werewolf $character, array $arcana): void
  // {
  ...
  //         $newArcanum = new WerewolfArcanum($character, $arcanum, (int)$level);
  //       }
  //       // else {
  ...
  /** @param array<int, int> $arcana */
  // public function addRotes(Werewolf $character, array $rotes): void
  // {
  ...
  {
    $rules = $chronicle->getRules('werewolf');

    // // We do not have any custom rules, or no custom rules for werewolf
    if (is_null($rules)) {
      $rules = [
        'maxEssence' =>  [
          '1' => 10,
